...
func (d *TestStaticDirectoryServer) DeleteTenant(tenantID roachpb.TenantID) {
	...
	// Emit an event that the tenant has been deleted. Since the tenant has been
	// deleted, there's no point emitting other fields (e.g. ClusterName).
	d.notifyTenantUpdateLocked(&tenant.Tenant{
		TenantID: tenantID.ToUint64(),
		State:    tenant.TENANT_DELETING,
	})
}
